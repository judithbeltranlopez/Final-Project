...
ggplot() +
  ...
  coord_sf(crs = st_crs(galway_sf)) + theme_bw() +
  theme(
...
# constants to scale density onto "count" axis
n_towns   <- nrow(df_active_pct_town)
binwidth  <- diff(range(df_active_pct_town$pct_active, na.rm = TRUE)) / 30  # matches bins = 30
