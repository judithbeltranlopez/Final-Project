...
```
```{r standard-plot2, out.width='90%', fig.align='center', fig.cap='School Population', fig.height=4,warning=FALSE, message=FALSE}
library(janitor)
library(tidyverse)
library(dplyr)
library(sf)
...
schools_NA <- schools_df[complete.cases(schools_df$`School Latitude`,schools_df$`School Longitude`), ]
schools_sf <- st_as_sf(schools_NA,coords = c("School Longitude", "School Latitude"),crs = 4326)
ireland <- ne_countries(scale = "medium",country = "Ireland",returnclass = "sf")
ggplot() + geom_sf(data = ireland) + geom_sf(data = schools_sf, color = "red", size = 1)+theme_void()
...
```{r standard-plot3, out.width='80%', fig.align='center', fig.cap='Kriging schools in Co Galway', fig.height=4,warning=FALSE,message=FALSE}
...
sink(tempfile())
kriged_sp <- suppressMessages(krige(
  formula   = log_enrol ~ 1,
