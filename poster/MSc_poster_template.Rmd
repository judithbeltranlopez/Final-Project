...
```{r standard-plot4, out.width='95%', fig.align='center', fig.cap='Kriging schools in Co Galway', fig.height=4,warning=FALSE,message=FALSE}
...
df_active_town <- df_clean %>%
  filter(means_of_travel %in% c("On Foot", "Bicycle")) %>%
  group_by(towns_2022) %>%
  summarise(total_active = sum(value), .groups = "drop")

ggplot(df_active_town, aes(x = total_active)) +
  geom_histogram(
    bins = 30,
    fill = "steelblue",
    color = "white"
  ...
  scale_x_log10() +
  labs(
    title = "Distribution of Total Active Transport Users Across Towns",
    x = "Total active-mode users (log scale)",
    y = "Number of towns"
  ) +
  theme_minimal()
